...
2. **Macro `bao_dong`:** 
   - Lệnh 1: Kiểm tra điều kiện (If): `Variable alarm == 1`.
   - Lệnh 2: Bật Relay còi báo động.
   - Lệnh 3: Delay **30** (30 giây).
   - Lệnh 4: Tắt Relay còi báo động.
3. **Rule cửa cổng:** 
   - Sự kiện kích hoạt: Khi công tắc từ cửa cổng chuyển sang trạng thái **Mở (Open)**.
   - Hành động: Gọi macro `bao_dong`.

**Nguyên lý hoạt động:** Khi cửa mở, Rule sẽ gọi Macro `bao_dong`. Bên trong Macro, lệnh đầu tiên sẽ kiểm tra biến `alarm`. Nếu anh đang bật báo động (`alarm = 1`), còi sẽ hú 30s rồi tắt. Nếu `alarm = 0`, Macro sẽ dừng ngay tại lệnh kiểm tra và không hú còi.
