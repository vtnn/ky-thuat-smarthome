...
- Đấu đúng ngõ vào Dry Contact — tuyệt đối không đưa 220V vào ngõ vào.
...
Khác với các module thông thường, relay trên DIN-RY8-N được thiết kế để hoạt động tương tự như công tắc 2 chiều (công tắc đảo). Điều này cho phép hệ thống smarthome can thiệp vào mạch đèn mà vẫn giữ nguyên khả năng điều khiển bằng công tắc cơ tại chỗ.

### 2.1. Cách đấu công tắc cơ (mạch đảo 2 chiều)

Để ứng dụng và công tắc tay luôn đồng bộ trạng thái, ta bắt buộc phải đấu theo sơ đồ mạch đảo. Nguyên tắc nằm lòng: lửa cấp cho công tắc, tải lấy từ relay.
...
- Chân L (chân chung): đấu dây lửa trực tiếp từ nguồn điện lưới.
- Chân 1 và 2: đấu 2 sợi dây đảo (cos vàng) để kéo về tủ điện, nối vào cặp L1-L2 đã chờ sẵn.

Sự kết hợp này biến relay smarthome và công tắc cơ thành một "cặp bài trùng" đảo chiều cho nhau. Bạn bật trên ứng dụng thì đèn sáng, ra nhấn công tắc tay thì đèn tắt — hiện đại mà vẫn giữ nguyên thói quen dùng cơ cho chủ nhà.

> Cảnh báo sai sót: Nhiều thợ quen tay đấu lửa trực tiếp vào relay C. Kết quả là đèn vẫn sáng nhưng ứng dụng sẽ báo trạng thái "luôn bật" vì ứng dụng không hiểu được trạng thái của công tắc cơ đang đóng hay mở ở đầu kia. Đấu đúng chuẩn giúp ứng dụng báo trạng thái chuẩn 100%.

### 2.2. Quy trình kiểm tra chuẩn (bắt buộc)

Sau khi đấu nối, kỹ thuật viên bắt buộc phải thực hiện đúng quy trình 4 bước sau để xác nhận mạch đảo hoạt động đúng:

1. Bật công tắc cơ → đèn phải sáng.
2. Bấm nút trên ứng dụng (hoặc trên module) → đèn phải tắt.
3. Bật lại công tắc cơ → đèn phải sáng.
4. Bấm nút trên ứng dụng (hoặc trên module) → đèn phải tắt.

Nếu thực hiện xuyên suốt 4 bước mà đèn phản hồi chính xác như trên, nghĩa là đã đấu nối đúng sơ đồ mạch đảo. Đây là quy trình chuẩn bắt buộc phải thực hiện và nghiệm thu trước khi bàn giao tủ. Nếu có bất kỳ bước nào đèn không chuyển trạng thái, phải kiểm tra lại dây cặp L1-L2 ngay.

---

## 3. Đấu nối ngõ vào Dry Contact

Ngõ vào Dry Contact nhận tín hiệu từ công tắc cơ, cảm biến chuyển động hồng ngoại, công tắc từ cửa.

- Chân [–] là COM chung cho toàn bộ 8 kênh ngõ vào.
- Chân [1] đến [8] là từng kênh riêng biệt.
- Tín hiệu vào phải là tiếp điểm khô — tức là chỉ đóng/ngắt mạch, không có điện áp.

Cấp nhầm 220V vào chân ngõ vào sẽ cháy bo mạch. Không sửa được, phải thay bo mới. Đây là lỗi đã xảy ra thực tế, đặc biệt khi thợ điện không quen với hệ thống smarthome và tưởng ngõ vào cũng giống ngõ vào điện bình thường.
...
- [ ] Ngõ vào chỉ nhận tiếp điểm khô, tuyệt đối không đưa 220V vào.
- [ ] Nguồn 24VDC đúng cực (+/-). Mỗi tủ dùng nguồn riêng biệt.
- [ ] Dây mạng kéo ra mắt hồng ngoại (IR) đã được dán nhãn "KHÔNG CẮT, THÁO RA". Phải đo kiểm bằng đồng hồ (thang đo một chiều) đảm bảo không lọt áp 24V sang chân tín hiệu Tx+/Tx- trước khi cắm nối module.
