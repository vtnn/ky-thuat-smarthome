...
1. **Chuẩn bị**: Tải ứng dụng LifeSmart. Cân nhắc cắm dây cáp mạng LAN vào Smart Station để đảm bảo kết nối ổn định nhất. Điện thoại của bạn phải đang kết nối vào cùng mạng Wi-Fi (cùng mạng LAN) với Smart Station.
2. **Quét thiết bị**: Trong ứng dụng, nhấn dấu `+` → Chọn **Smart Station** → Chọn **Search (Khuyến nghị)**. Ứng dụng sẽ tự quét mạng nội bộ và liệt kê các Hub đang online.
3. **Thêm thiết bị**: Chọn Smart Station hiển thị trong danh sách để thêm vào tài khoản.
4. **Cập nhật Firmware (Bắt buộc)**: Ngay sau khi thêm thành công, anh em phải vào **Setting → Advanced Settings → Update Smart Device** để cập nhật Smart Station lên bản Firmware mới nhất. Bước này rất quan trọng để Hub nhận diện được các thiết bị con đời mới và chạy kịch bản ổn định.
...
5. Vào danh sách thiết bị, chọn HVAC Gateway, vào phần cài đặt.
6. Trượt màn hình từ phải sang trái để bắt đầu đồng bộ nhóm địa chỉ (Sync). Chờ 1–10 phút. Trong lúc đang đồng bộ, không thao tác gì trên HVAC Gateway.
7. Chọn các nhóm dàn lạnh để đưa về màn hình hiển thị dưới dạng điều khiển máy lạnh riêng biệt. Đặt lại tên từng máy lạnh theo phòng.
