...
|---|---|---|
| Hub mất kết nối | Lỗi mạng dây/WiFi, router mất mạng | Khởi động lại router → rút cắm lại Hub |
| Thiết bị mất kết nối | Pin yếu, quá xa Hub, mất nguồn 220V | Thay pin / kiểm tra CB / dời Hub gần hơn |
| Ghép nối thất bại | Chưa đưa thiết bị vào chế độ ghép nối, đứng quá xa Hub | Nhấn giữ nút lại, đưa thiết bị gần Hub dưới 5m |
| DEFED đèn vàng | Cảm biến kích hoạt khi hệ thống đang tắt giám sát, hoặc Hub mất kết nối | Kiểm tra trạng thái Hub + danh sách cảm biến |
...
| App hiện "Device Offline" | Mất sóng CoSS, Hub chưa cập nhật firmware, pin hết | Kiểm tra sóng, cập nhật firmware Hub, thay pin |
...
- Đèn trạng thái trên Hub có sáng không?
...
Hub LifeSmart cần kết nối mạng ổn định:
- Router có online không? Thử ping Google từ điện thoại cùng WiFi.
- Hub nối dây LAN hay WiFi? Nếu LAN thì đổi cổng, đổi dây thử. Nếu WiFi thì kiểm tra tín hiệu.
- Có ai vừa đổi mật khẩu WiFi không? (Hub kết nối WiFi sẽ bị đá ra nếu đổi pass.)

### Bước 3 — Kiểm tra trên app

Mở ứng dụng LifeSmart:
- Hub có hiện online không?
- Thiết bị có hiện "Offline" không?
- Firmware Hub đã cập nhật mới nhất chưa? (Setting → Advanced Settings → Update Smart Device)

### Bước 4 — Khởi động lại

Nếu 3 bước trên không phát hiện lỗi rõ ràng:
1. Khởi động lại router (rút nguồn 10 giây, cắm lại).
2. Khởi động lại Hub (rút nguồn 10 giây, cắm lại).
3. Đợi 2-3 phút cho Hub reconnect.
4. Kiểm tra lại trên app.

Thứ tự quan trọng: router trước, Hub sau. Nếu khởi động Hub trước mà router chưa lên thì Hub vẫn không kết nối.

### Bước 5 — Cô lập lỗi

Nếu khởi động lại không giải quyết:
- Thử điều khiển thiết bị khác trên cùng Hub. Nếu thiết bị khác vẫn hoạt động → lỗi nằm ở thiết bị cụ thể, không phải Hub.
- Thử ghép nối lại thiết bị lỗi (đưa gần Hub, nhấn giữ nút ghép nối).
- Nếu tất cả thiết bị đều lỗi → có thể Hub hỏng hoặc mạng có vấn đề sâu hơn.

---

## Hub và thiết bị mất kết nối

### Hub mất kết nối

Khởi động lại router trước, sau đó rút nguồn Hub khoảng 10 giây rồi cắm lại. Nếu vẫn không lên:
- Kiểm tra cổng mạng: thử đổi cổng trên router hoặc đổi dây mạng.
- Nếu Hub dùng WiFi: kiểm tra xem có ai đổi mật khẩu WiFi, hoặc router có chuyển băng tần không (LifeSmart Smart Station chỉ hỗ trợ 2.4GHz).
- Nếu đèn Hub sáng bình thường nhưng app vẫn báo offline: có thể do firewall trên router chặn traffic ra ngoài. Thử tắt firewall tạm hoặc thêm Hub vào whitelist.
...
- Sau khi thay pin, nhiều thiết bị cần 1-2 phút để tự kết nối lại với Hub.
...
Sóng yếu:
- Dời Hub ra vị trí thoáng hơn, tránh đặt trong tủ điện kim loại kín.
- Nếu nhà lớn hoặc nhiều tầng: thêm Hub phụ (Cascade) và chia thiết bị theo khu vực.
...
Đèn vàng: có cảm biến kích hoạt khi hệ thống đang ở chế độ tắt giám sát, hoặc Hub mất kết nối. Không nguy hiểm nhưng cần kiểm tra:
- Mở app, vào trang DEFED, xem cảm biến nào đang báo kích hoạt.
- Nếu là cảm biến cửa: kiểm tra cửa đã đóng hẳn chưa.
- Nếu là Hub mất kết nối: xử lý theo mục Hub ở trên.
...
1. Kịch bản đã bật chưa? Trên trang Smart, mỗi kịch bản có toggle bật/tắt. Đôi khi khách hàng vô tình tắt.
...
4. Múi giờ Smart Station có đúng UTC+7 không? Nếu sai, các trigger hẹn giờ sẽ chạy lệch.
...
### Kịch bản chạy khi mất mạng thì không

Kịch bản mặc định chạy qua cloud. Khi mất internet, cloud không nhận trigger, action không gửi được. Giải pháp: chuyển sang CoSSLink (chạy local) nếu trigger và action đều nằm trong cùng Smart Station.
...
Nếu không tìm thấy chức năng Replace: kiểm tra firmware Hub. Các phiên bản firmware cũ có thể chưa hỗ trợ tính năng này — cập nhật firmware trước.
...
- Hub khởi động không lên dù đã thử mọi cách
...
- [ ] Kiểm tra vật lý: CB, nguồn điện, dây mạng, đèn trạng thái Hub
- [ ] Kiểm tra mạng: router online, Hub online trên app
- [ ] Khởi động lại: router → Hub (đúng thứ tự)
